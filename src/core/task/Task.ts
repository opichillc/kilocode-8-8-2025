...
export class Task extends EventEmitter<TaskEvents> {
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		while (!this.abort) {
			//const didEndLoop = await this.recursivelyMakeClineRequests(nextUserContent, includeFileDetails) // kilocode_change
			const didEndLoop = await this.makeRequest(nextUserContent, includeFileDetails) // kilocode_change use non recursive version
	...
	public async makeRequest(
		...
		while (stack.length > 0) {
			...
			if (frame.stage === "start") {
				if (this.abort) {
					throw new Error(`[KiloCode#request] task ${this.taskId}.${this.instanceId} aborted`)
				}
				...
				const environmentDetails = await getEnvironmentDetails(this, frame.includeFileDetails)
